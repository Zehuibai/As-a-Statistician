...
```
packages<-c("assertr","stats","tidyr", "stringi")
ipak <- function(pkg){
...
### Get date for data name
current.date <- ymd(format(Sys.Date(), '%Y-%m-%d'))

### Get the path
get.path <- dirname(rstudioapi::getSourceEditorContext()$path)
### Read csv data
library(readr)
Readdata <- read_delim(file = file.path(get.path, "name.csv"), delim = ";", escape_double = FALSE, trim_ws = TRUE, skip = 1)
```
